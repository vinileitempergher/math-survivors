...
# EXPLICAÇÕES POR TEMA (resumidas)
var quiz_topic_explanations = {
	"porcentagem": "Porcentagem é uma parte de 100.\nPara calcular: número × (percentual / 100).\nEx: 30% de 200 = 200 × 0,30 = 60.",
	"notacao_cientifica": "Usamos notação científica para números muito grandes ou pequenos.\nForma: a × 10^n, com 1 ≤ a < 10.\nEx: 3000 = 3 × 10^3.",
	"potenciacao": "Potência é multiplicar o mesmo número várias vezes.\nEx: 2^3 = 2 × 2 × 2.\nRegra: a^m × a^n = a^(m+n).",
	"raiz": "Raiz quadrada: número que vezes ele mesmo dá o valor.\nEx: √81 = 9 → 9 × 9 = 81.\nRaiz cúbica: número que vezes ele mesmo 3 vezes dá o valor.",
	"area_perimetro": "Área mede o espaço dentro da figura.\nEx: retângulo = base × altura.\nPerímetro é a soma de todos os lados.",
	"inteiros": "Inteiros incluem negativos, zero e positivos.\nSoma: sinais iguais somam; diferentes subtraem.\nMultiplicação/divisão: sinais iguais → positivo; diferentes → negativo.",
	"algebra": "Em álgebra usamos letras para representar números.\nTroque a letra pelo valor e faça as contas na ordem certa.",
	"equacao_1_grau": "Equações de 1º grau têm x só na primeira potência.\nIdeia: deixar o x sozinho.\nEx: 2x + 1 = 11 → 2x = 10 → x = 5.",
	"sistema_equacoes": "Sistema é um conjunto de equações com as mesmas letras.\nSomar/subtrair as equações ajuda a eliminar uma letra e achar os valores.",
	"estatistica": "Média: soma ÷ quantidade.\nMediana: valor do meio (lista em ordem).\nModa: valor que mais se repete.",
	"geometria": "Polígonos: figuras com vários lados.\nSoma dos ângulos internos: (n - 2) × 180°.\nDiagonais: n(n - 3) / 2.",
	"geral": "Leia com calma, identifique o tipo de conta (porcentagem, potência, área, etc.) e só depois responda."
}

# Nomes bonitos para mostrar no painel
var quiz_topic_names = {
	"porcentagem": "Porcentagem",
	"notacao_cientifica": "Notação Científica",
	"potenciacao": "Potenciação",
	"raiz": "Raiz",
	"area_perimetro": "Área e Perímetro",
	"inteiros": "Números Inteiros",
	"algebra": "Álgebra",
	"equacao_1_grau": "Equações de 1º Grau",
...
func show_quiz_difficulty_selection():
	...
	difficulty_panel.set_anchor(SIDE_TOP, 0.15)     
	difficulty_panel.set_anchor(SIDE_BOTTOM, 0.85)  
	...
	options_container.offset_top = 45.0
	...
	easy_btn.text = "🟢 Fácil"
	easy_btn.custom_minimum_size = Vector2(0, 35)
...
func show_quiz():
	...
	question_label.custom_minimum_size = Vector2(0, 50)
	...
	for option in shuffled_options:
		...
		option_btn.custom_minimum_size = Vector2(0, 40)
...
# Detecta o tema da pergunta pelo texto
func get_topic_from_question(question_text: String) -> String:
	question_text = question_text.strip_edges()

	if question_text.begins_with("Porcentagem"):
		return "porcentagem"
	elif question_text.begins_with("Notação científica"):
		return "notacao_cientifica"
	elif question_text.begins_with("Potenciação"):
		return "potenciacao"
	elif question_text.begins_with("Raiz quadrada") or question_text.begins_with("Raiz cúbica") or question_text.begins_with("Raiz:"):
		return "raiz"
	elif question_text.begins_with("Área") or question_text.begins_with("Perímetro"):
		return "area_perimetro"
	elif question_text.begins_with("Inteiros"):
		return "inteiros"
	elif question_text.begins_with("Álgebra"):
		return "algebra"
	elif question_text.begins_with("Equação"):
		return "equacao_1_grau"
	elif question_text.begins_with("Sistema"):
		return "sistema_equacoes"
	elif question_text.begins_with("Estatística"):
		return "estatistica"
	elif question_text.begins_with("Geometria"):
		return "geometria"
	else:
		return "geral"

# Verifica resposta e decide se mostra level up ou explicação
func _on_quiz_answer_selected(selected_answer):
	...
	if normalized_selected == normalized_answer:
		...
		show_quiz_explanation(selected_answer)

# Painel com “você errou” + explicação do conteúdo
func show_quiz_explanation(selected_answer):
	var topic = get_topic_from_question(current_quiz_question["question"])
	var explanation = quiz_topic_explanations.get(topic, quiz_topic_explanations.get("geral"))
	var topic_nice_name = quiz_topic_names.get(topic, topic.capitalize())

	var explanation_panel = Panel.new()
	explanation_panel.name = "QuizExplanationPanel"
	explanation_panel.process_mode = Node.PROCESS_MODE_ALWAYS
	explanation_panel.visible = true

	# Painel maior
	explanation_panel.set_anchor(SIDE_LEFT, 0.18)
	explanation_panel.set_anchor(SIDE_RIGHT, 0.82)
	explanation_panel.set_anchor(SIDE_TOP, 0.18)
	explanation_panel.set_anchor(SIDE_BOTTOM, 0.82)

	var gui_layer = get_node("GUILayer/GUI")
	gui_layer.add_child(explanation_panel)
	...
	explanation_panel.add_child(margin)
	...
	var title = Label.new()
	title.text = "Você errou a questão!"
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title.add_theme_font_override("font", preload("res://Font/tenderness.otf"))
	...
	answer_label.text = "Resposta correta: %s" % str(current_quiz_question["answer"])
	answer_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	answer_label.add_theme_font_override("font", preload("res://Font/tenderness.otf"))
	answer_label.add_theme_font_size_override("font_size", 14)
	vbox.add_child(answer_label)

	var topic_label = Label.new()
	topic_label.text = "Conteúdo: %s" % topic_nice_name
	topic_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	topic_label.add_theme_font_override("font", preload("res://Font/tenderness.otf"))
	...
	expl_label.text = explanation
	expl_label.autowrap_mode = TextServer.AUTOWRAP_WORD
	expl_label.size_flags_vertical = Control.SIZE_EXPAND_FILL
	expl_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	expl_label.add_theme_font_override("font", preload("res://Font/tenderness.otf"))
	expl_label.add_theme_font_size_override("font_size", 13)
	vbox.add_child(expl_label)
	...
	button_row.add_child(left_spacer)

	var ok_btn = Button.new()
	ok_btn.text = "Entendi"
	ok_btn.custom_minimum_size = Vector2(140, 42)
	ok_btn.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	ok_btn.add_theme_font_override("font", preload("res://Font/tenderness.otf"))
	ok_btn.add_theme_font_size_override("font_size", 14)
	ok_btn.pressed.connect(_on_quiz_explanation_ok_pressed.bind(explanation_panel))
	button_row.add_child(ok_btn)

	var right_spacer = Control.new()
	right_spacer.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	button_row.add_child(right_spacer)

	get_tree().paused = true

func _on_quiz_explanation_ok_pressed(panel: Panel):
...
func _finalize_quiz_layout_paths(panel_path: NodePath, label_path: NodePath, options_path: NodePath) -> void:
